...
class PushNotificationsService {
      ...
      if (isAndroid) {
        if (await PermissionsService.isDrawOverlaysPermisisonGranted()) {
          ...
          const dummyCallSession = {
            initiatorID: notification.payload.initiatorId,
            opponentsIDs: notification.payload.opponentsIds.split(','),
            ID: notification.payload.uuid,
          };
  ...
  subscribeToPushNotifications(deviceToken) {
    const params = {
      notification_channel: isIOS ? 'apns' : 'gcm',
      device: {
        platform: platformOS,
        udid: getUniqueId(),
      },
  ...
  deleteSubscription() {
    ...
    ConnectyCube.pushnotifications.subscriptions.list().then(result => {
      for (let item of result) {
        ...
        if (subscription.device.platform === platformOS && subscription.device.udid === deviceUdid) {
